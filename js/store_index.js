...
// 函数封装 --------------------------------------
	...
	// 导航栏弹性设置margin
		function navMar() {
			...
			$('.store_index_nav_inner li').each( function() {
				data += $(this).width();
			} );
			
			target = Math.floor( ( $('.store_index_nav_inner').width() - data )/3 ) -1;

			$('.store_index_nav_inner li').css('margin-right' , target+'px')
		}

	// 启用轮播图
		var mainCarousel = new Swiper( '.store_index_carousel_inner' , {
			autoplay: 3000 ,
			loop: true,
